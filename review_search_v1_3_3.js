/* 공진단 우피 - 우피 DOM 변경 대응 및 날짜/정렬/모바일 2열 보완 v1.3.3 */
(() => {
  "use strict";
  ...
  const CARD_TITLE_SELECTOR =
    '[data-root="true"][placeholder="Untitled"], [data-root="true"][contenteditable="false"]';
  const CARD_TILE_SELECTOR =
    '[role="button"][aria-label="collection tile"]';
  const CARD_SLOT_SELECTOR = ".notion-collection-item";
  ...
  console.info("[공진단 리뷰] v1.3.3 로드됨");
  ...
  /**
   * .notion-collection-item은 우피의 안정적인 카드 슬롯입니다.
   * 임시 해시 클래스가 바뀌어도 같은 부모 아래의 슬롯을 하나의 갤러리로 묶습니다.
   */
  function discoverGalleryGroups() {
    ...
    items.forEach((item) => {
      const container = item.slot?.parentElement;
      if (!container) return;

      if (!groups.has(container)) {
        groups.set(container, new Map());
      }

      groups.get(container).set(item.slot, item);
    });
  ...
  function getRawCardItems() {
    ...
    [...document.querySelectorAll(CARD_TILE_SELECTOR)].forEach((inner) => {
      if (inner.closest(`#${UI_ID}`)) return;

      const slot = inner.closest(CARD_SLOT_SELECTOR);
      const titleElement = inner.querySelector(CARD_TITLE_SELECTOR);
      if (!slot || !titleElement || seenSlots.has(slot)) return;
      if (!normalizeText(titleElement.textContent)) return;

      seenSlots.add(slot);
      items.push({ titleElement, inner, slot });
    });
