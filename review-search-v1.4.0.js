/* 공진단 우피 - 리뷰 검색·최신순 정렬·날짜 표시 v1.4.0 */
(() => {
  "use strict";
  ...
  function parseReviewDate(value) {
    ...
    /*
     * 2026/02/31처럼 실제로 존재하지 않는 날짜는 제외합니다.
     */
    if (
  ...
  /**
   * 카드 속성 영역에서 날짜만 들어 있는 가장 안쪽 텍스트를 찾습니다.
   */
  function findReviewDateTarget(root) {
    if (!root) return null;
    ...
    while (textNode) {
      const parsed = parseReviewDate(textNode.nodeValue);

      if (parsed) {
        textCandidates.push({
          type: "text",
          node: textNode,
          rawValue: textNode.nodeValue,
          parsed,
        });
      }
    ...
    /*
     * 날짜와 시간이 여러 span으로 나뉘어 있을 때의 보조 처리입니다.
     * 가장 안쪽에서 정확히 날짜 형식과 일치하는 요소만 선택합니다.
     */
    const elements = [root, ...root.querySelectorAll("*")].reverse();

    for (const element of elements) {
      const parsed = parseReviewDate(element.textContent);
      if (!parsed) continue;

      const childHasDate = [...element.children].some((child) =>
        Boolean(parseReviewDate(child.textContent))
      );
  ...
  function replaceDateWithDateOnly(target, formattedDate) {
    if (!target) return;

    if (target.type === "text") {
      const rawValue = String(target.rawValue || "");
      const leadingSpace = rawValue.match(/^\s*/)?.[0] || "";
      const trailingSpace = rawValue.match(/\s*$/)?.[0] || "";
      const nextValue = `${leadingSpace}${formattedDate}${trailingSpace}`;
    ...
    if (
      target.type === "element" &&
      target.element.textContent.trim() !== formattedDate
    ) {
      target.element.textContent = formattedDate;
    }
  }

  /**
   * 화면에는 날짜만 남기되, 최초에 읽은 시간 정보는 정렬용으로 보존합니다.
   */
  function getAndFormatCardDate(cardElement, cardInner) {
    const target = findReviewDateTarget(
      cardInner?.querySelector(".css-1yjhumr") || cardInner
    );
    ...
    replaceDateWithDateOnly(target, current.formattedDate);

    return dateInfo;
  }

  /**
   * 각 갤러리 안에서 작성일자가 최신인 카드부터 보이도록 order 값을 지정합니다.
   * 날짜를 찾지 못한 카드는 기존 순서를 유지한 채 맨 뒤로 이동합니다.
   */
  function sortAndFormatReviewCards(groups) {
    groups.forEach(({ cards }) => {
      const sortedCards = cards
        .map((card, originalIndex) => {
          const dateInfo = getAndFormatCardDate(card.element, card.inner);

          return {
            ...card,
            originalIndex,
            timestamp: dateInfo?.timestamp ?? Number.NEGATIVE_INFINITY,
      ...
      sortedCards.forEach((card, order) => {
        card.element.style.setProperty("--gj-review-order", String(order));

        if (card.dateKey) {
          card.element.setAttribute("data-gj-review-date", card.dateKey);
        } else {
          card.element.removeAttribute("data-gj-review-date");
        }
      });
    });
  }
  ...
  function applyReviewGalleryLayout() {
    ...
    sortAndFormatReviewCards(groups);
